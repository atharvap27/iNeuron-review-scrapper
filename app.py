...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            db = dbConn['FlipkartDB'] # connecting to the database called crawlerDB
            ...
            if reviews.count() > 0: # if there is a collection with searched keyword and it has records in it
                ...
                req_url = uReq(flipkart_url) # requesting the webpage from the internet
                mainPage = req_url.read() # reading the webpage
                req_url.close() # closing the connection to the web server
                flipkart_html = bs(mainPage, "html.parser") # parsing the webpage as HTML
